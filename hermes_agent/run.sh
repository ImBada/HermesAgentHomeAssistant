...
for env_name, option_name in secret_options.items():
    value = text(option_name)
    if value:
        env_values[env_name] = value

custom_api_key = text("custom_api_key")
if custom_api_key:
    env_values["OPENAI_API_KEY"] = custom_api_key

env_lines = [
    "# Generated by the Home Assistant Hermes Agent add-on.",
    "# Edit add-on options instead of this file; it is rewritten on start.",
]
...
model_config = {
    "default": text("model", "anthropic/claude-opus-4.6"),
    "provider": provider,
}
...
if personality:
    (home / "SOUL.md").write_text(personality + "\n", encoding="utf-8")
...
runtime = {
    ...
    "dashboard_port": validated_port("dashboard_port", 9119),
...
DASHBOARD_PORT="$(read_runtime '.dashboard_port')"
